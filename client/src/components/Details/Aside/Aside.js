...
import { createHire } from '../../../services/productData';
...
function Aside({ params, history }) {
    ...
    const handleHire = () => setShowHire(true);
    ...
    const onCreateHire = (e) => {
        e.preventDefault();
        let product_id = params._id
        let obj = { product_id,phone, location}
        createHire(obj)
            .then((res) => {
                history.push('/')
            })
    ...
    return (
        <aside>
            <div className="product-details-seller">
                ...
                {params.isAuth ? (<>
                    ...
                    <>
                        ...
                        <Button variant="dark" className="col-lg-10" id="btnContact" onClick={handleHire}>
                            <RiMessage3Fill />Hire Seller
                        </Button>
                    </>
            ...
            <Modal show={showHire} onHide={handleCloseHire}>
                <Modal.Header closeButton>
                    <Modal.Title>Hire Seller</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form>
                        <Form.Group controlId="">
                            <Form.Label>Phone number</Form.Label>
                            <Form.Control type="phone" name="phone" placeholder="Enter phone" required onChange={e=>setPhone(e.target.value)} />
                        </Form.Group>
                        <Form.Group controlId="">
                            <Form.Label>Location</Form.Label>
                            <Form.Control type="location" name="location" placeholder="Enter location" required onChange={e=>setLocation(e.target.value)} />
                        </Form.Group>
                    </Form>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="dark" onClick={onCreateHire}>Sent</Button>
