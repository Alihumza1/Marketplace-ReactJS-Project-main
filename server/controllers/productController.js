...
const createHire = require('../models/createHire');
...
router.get('/getHire', async (req, res) => {
    const { page, search } = req.query;
    try {
        let products;
        products = await createHire.paginate({}, { page: parseInt(page) || 1, limit: 5 });
        res.status(200).json({ products: products.docs, pages: products.pages });
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
...
router.post('/createHire', async (req, res) => {
    console.log('Shoaib')
    let { product_id, phone, location} = req.body;
    try {
        let errors = [];
        if (phone.length < 3 || phone.length > 50) errors.push('Phone should be at least 3 characters long and max 50 characters long; ');
        if (!location) errors.push('Location is required; ');

        if (errors.length >= 1) throw { message: [errors] };

        let product = new createHire({
            product_id,phone, location,
